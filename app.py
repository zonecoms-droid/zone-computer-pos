...
# ----------------------------------------------------
# 💻 โหมดที่ 2: หน้าหลักสำหรับเจ้าของร้าน (POS, สต็อก, จัดการซ่อม, สร้าง QR)
# ----------------------------------------------------
else:
    ...
    menu = st.sidebar.selectbox(
        ...
        [
            ...
            "📦 จัดการสต็อกสินค้า",
            "📝 ระบบจัดการงานซ่อม",
            "🖨️ สร้าง QR Code ให้ลูกค้าสแกนซ่อม",
            "📊 สรุปยอดขายรายวัน"
        ]
    )
...
========================================
    ...
    # 4. สร้าง QR Code สำหรับให้ลูกค้าสแกน
    elif menu == "🖨️ สร้าง QR Code ให้ลูกค้าสแกนซ่อม":
        st.subheader("🖨️ สร้าง QR Code สำหรับตั้งหน้าร้าน")
        st.write("คุณสามารถนำ QR Code นี้ไปปริ้นท์แปะหน้าร้าน หรือตั้งไว้ที่เคาน์เตอร์ เพื่อให้ลูกค้าสแกนกรอกข้อมูลแจ้งซ่อมด้วยตัวเองได้ทันที")
        
        # ช่องให้ใส่ลิงก์เว็บแอปจริงของคุณ (เช่น https://zone-computer-pos.streamlit.app)
        base_url = st.text_input("ระบุลิงก์เว็บไซต์แอปของคุณ (URL ของ Streamlit Cloud)", value="https://")
        
        if base_url and base_url != "https://":
            # สร้าง URL ที่พ่วง query parameter ?mode=register
            target_url = f"{base_url.strip('/')}/?mode=register"
            
            st.write(f"🔗 ลิงก์สำหรับสแกน: `{target_url}`")
            ...
            st.success("✅ สร้าง QR Code สำเร็จ! สามารถคลิกขวาที่รูปเพื่อบันทึกไปปริ้นท์ใช้งานได้เลยครับ")
        else:
            st.warning("⚠️ กรุณากรอกลิงก์เว็บไซต์แอปของคุณในช่องด้านบนก่อน ระบบถึงจะสร้าง QR Code ที่ถูกต้องได้")
